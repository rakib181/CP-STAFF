...
using namespace std;
using ll = long long;
const int N = 4e6 + 9, p1 = 131, p2 = 151, mod1 = 2147483647, mod2 = 999999937;
...
signed main(){
    ...
    for(int start = 0; start < k; start++){
        map<pair<int, int>, bool> vis; vector<int> ans;
        bool possible = true;
        for(int i = start, cnt = 0; cnt < n; i += k, cnt++){
            auto hs = hashing.get_hash(i, i + k - 1);
            if(!vis.count(hs) and id.count(hs)){
                vis[hs] = true;
                ans.emplace_back(id[hs]);
            }else{
                possible = false;
                break;
            }
