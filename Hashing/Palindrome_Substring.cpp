...
const int N = 1e3 + 9, p1 = 131, p2 = 151, mod1 = 2147483647, mod2 = 999999937;
...
signed main(){
    ...
    while (cin >> s) {
        ...
        h1.build(s);
        h2.build(rev);
        set<string> se; vector<pair<int, int>> coordinate;
        for (int i = 0; i < n; i++) {
            for (int j = i + 1; j < n; j++) {
                if (h1.get_hash(i, j) == h2.get_hash(n - j - 1, n - i - 1)) {
                    coordinate.emplace_back(i, j);
                }
            }
        }
        for (auto x : coordinate) {
            se.insert(s.substr(x.first, (x.second - x.first + 1)));
        }
        for (auto x : se) {
            cout << x << '\n';
        }
