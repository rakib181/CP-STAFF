...
struct HASH {
    ...
    void build(string &a) {
        ...
        int n = (int) a.size();
        for (int i = 0; i < n; i++) {
            prefix[i].first = 1LL * a[i] * pw[i].first % mod1;
            if (i) prefix[i].first = 1LL * (1LL * prefix[i].first + prefix[i - 1].first) % mod1;
            prefix[i].second = 1LL * a[i] * pw[i].second % mod2;
...
signed main(){
    ...
    for(int i = 0; i < n; i++){
        ...
        while(l <= r){
            int m = (l + r) >> 1;
            if(ori.get_hash(i + 1, i + m) == rev.get_hash(n - i, n - i + m - 1)){
                ans = m;
                l = m + 1;
            }else{
                r = m - 1;
    ...
    for(int i = 0; i < n; i++){
        ...
        while(l <= r){
            int m = (l + r) >> 1;
            if(ori.get_hash(i, i + m - 1) == rev.get_hash(n - i, n - i + m - 1)){
                ans = m;
                l = m + 1;
            }else{
                r = m - 1;
